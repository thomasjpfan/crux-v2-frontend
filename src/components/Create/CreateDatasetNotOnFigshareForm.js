...
class CreateDatasetNotOnFigshareForm extends Component {
  ...
  createDatasetOnFigshare = e => {
    ...
    const { title, description, licenseValue, tags } = this.state;
    ...
    MD5Promise(file)
      .then(data => {
        ...
        return fetch("https://api.figshare.com/v2/account/articles", {
          method: "POST",
          body: JSON.stringify({
            title: title,
      ...
      .then(data => {
        ...
        const partsPromises = parts.map(
          ({ startOffset, endOffset, partNo }) => {
      ...
      .then(() => {
        ...
        return fetch(
          `https://api.figshare.com/v2/account/articles/${newFigshareID}/files/${newFileID}`,
          {
            method: "POST",
            headers: authHeader
          }
      ...
      .then(() => {
        ...
        return fetch(
          `https://api.figshare.com/v2/account/articles/${newFigshareID}/private_links`,
          {
            method: "POST",
            headers: authHeader
          }
  ...
  updateCruxWithDataset = createDatasetOnCrux => () => {
    ...
    fetch(
      `https://api.figshare.com/v2/account/articles/${newFigshareID}/publish`,
    ...
    return (
      <Segment>
        <Form size="large" error={formErrorMessage !== ""}>
          <Form.Input
            label="Title"
            placeholder="Crux's Next Top Dataset"
            onChange={(e, { value }) => this.setState({ title: value })}
            value={title}
            error={formErrorLabel === "title"}
          />
          <Form.TextArea
            label="Description"
          ...
          <Form.Input
            label="Tasks (Press enter to add)"
            onKeyDown={this.addTaskKeyDown}
            error={formErrorLabel === "tasks"}
          />
          {tasks.length > 0 && (
            <List>
              {tasks.map((name, id) => (
          ...
          <div class="field">
            <label for="datasetFile">
              Choose a dataset to upload. Please zip files to upload more than
              one dataset.
            </label>
            <input
              type="file"
          ...
          <Form.Button primary onClick={this.openConfirmation}>
            Create Dataset
          </Form.Button>
          <Message
            error
        ...
        >
          ...
          <Modal.Actions>
            <Button
              color="red"
              onClick={() => this.setState({ modalOpen: "NONE" })}
            >
              <Icon name="remove" />
              No
            </Button>
        ...
        >
          ...
          {uploadToFigshareSucess && (
            <Modal.Actions>
              <Message
                size="large"
                header="Finished data upload to figshare"
                color="green"
              />
              <Button
                color="red"
                onClick={() => this.cancelUpload(() => {})}
                content="Cancel and try again"
              />
              <Button
                color="black"
              ...
              >
                {createDatasetOnCrux => (
                  <Button
                    primary
                    onClick={this.updateCruxWithDataset(createDatasetOnCrux)}
                    content="Finish uploading"
                  />
                )}
              </AddDatasetToCrux>
            </Modal.Actions>
...
export default withRouter(
  connect(mapStateToProps)(CreateDatasetNotOnFigshareForm)
);
