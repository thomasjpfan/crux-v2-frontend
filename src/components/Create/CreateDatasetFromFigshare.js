...
class CreateDatasetFromFigshare extends Component {
  ...
  loadDataFromFigshare = e => {
    ...
    if (!figshareURL.startsWith("https://figshare.com/articles")) {
      this.setState({
        figshareURLFormErrorMsg:
          "URL must start with https://figshare.com/articles"
      });
      return;
    }
    ...
    fetch(getFigshareInfoURL)
      ...
      .then(data => {
        ...
        const parsedDescription = description.replace(/(<([^>]+)>)/gi, "");
        this.setState({
      ...
      .catch(e => {
        this.setState({
          figshareURLFormErrorMsg:
            "Unable to get information from FigShare. Please confirm the url is correct"
        });
      })
      .finally(() => {
  ...
  render() {
    ...
    return (
      <Fragment>
        <Segment size="large">
          <Form
            onSubmit={this.loadDataFromFigshare}
            loading={figshareURLFormLoading}
            error={figshareURLFormHasError}
          >
            <Form.Input
              label="Enter Figshare URL"
